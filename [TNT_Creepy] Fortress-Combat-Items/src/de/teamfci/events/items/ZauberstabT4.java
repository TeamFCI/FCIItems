...
public class ZauberstabT4 implements Listener {
	...
	public void onInteract(PlayerInteractEvent ev) {
		...
		if (p.getItemInHand().getItemMeta().hasLore()) {
				if (p.getItemInHand().getItemMeta().getLore().get(2).equalsIgnoreCase("񙓬Anpassungsf鋒ig")) {
	...
	private void Right(final Player p) {
		...
		if (p.getItemInHand().getItemMeta().hasLore()) {
					if (hm.containsKey(p.getName() + "timer")) {
						p.sendMessage("Warte bis dein MagicArmor jemanden getroffen hat!");
						return;
					}
